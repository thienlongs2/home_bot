...
def load_api_keys():
    ...
    try:
        with open(API_KEYS_FILE, "r") as f:
            for line in f:
                account, key = line.strip().split(",")
                api_keys[account] = key
    except FileNotFoundError:
        print("File API keys không tồn tại!")
    return api_keys

API_KEYS = load_api_keys()

def authorize_request(mt5_account: str = Header(None), api_key: str = Header(None)):
    if mt5_account not in API_KEYS or API_KEYS[mt5_account] != api_key:
...
# Endpoint để lấy tín hiệu (chỉ cho phép tài khoản MT5 được ủy quyền)
@app.get("/get_signal")
async def get_signal(mt5_account: str = Depends(authorize_request)):
